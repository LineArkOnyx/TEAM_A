...
void AnimeClass::Draw(float x, float y, int type, float size,float angle,bool turn)
{
	//使用中なら現在のアニメ番号で描画する
	if (animeInfo[type].isUse)
	{
		...
		DrawRotaGraphF(x, y, size, angle, imageHundle, true, turn);
	}
}
